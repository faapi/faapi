...
faapi 作为独立服务运行,用 `faapi` 命令启动。默认扫描 `src/api/**/*.ts`。
...
- **路由根目录**:CLI 默认 `src/api/`,文件名 `handler.ts`(`--app-dir .` 可回退到根目录 `api/`)
- **中间件文件**:`middlewares.ts`,与 `handler.ts` 同目录或父目录,默认导出中间件数组
- **包管理器**:pnpm
- **模块系统**:ESM,`moduleResolution: Bundler`,本地相对导入路径不写后缀(由 tsc/tsup/esbuild 解析)
- **dev 启动**:`faapi` / `faapi dev`(默认扫描 `src/api/**/*.ts`,esbuild 编译到 `.faapi/dev/*.js`,不跑 tsc)
- **prod 启动**:`faapi start`(读 `dist/faapi-routes.js` 路由清单 + `dist/faapi-schema.js` schema,不依赖 NODE_ENV)
- **构建**:`faapi build`(用 esbuild 编译,生成 `dist/*.js` + `dist/faapi-routes.js` + `dist/faapi-schema.js`)
...
| 维度 | dev | prod |
|------|-----|------|
| 启动命令 | `faapi` / `faapi dev` | `faapi start`(需先 `faapi build`) |
| 文件类型 | `.ts`(esbuild 编译到 `.faapi/dev/*.js`) | `.js`(已编译) |
| 路由来源 | 扫描 `.ts`(`scanRoutes`)+ 加载 `.faapi/dev/*.js` | 读 `dist/faapi-routes.js` 清单 + 水合中间件 |
| schema 来源 | 预生成 `.faapi/dev/faapi-schema.js` | 加载 `dist/faapi-schema.js` |
| NODE_ENV | 兜底设为 `development`(未设置时) | 兜底设为 `production`(未设置时) |
| watch 模式 | ✅ 文件变化增量编译 + 热替换路由 | ❌ |
| tsc 检查 | ❌(用户自己跑) | ❌(应在 CI 已检查) |
| 插件加载时机 | server.listen 之前 | server.listen 之前 |

> dev/prd 模式由命令词决定,不再通过 `NODE_ENV=production` 切换。`NODE_ENV`/`FAAPI_ENV` 仅用于加载环境配置文件(`faapi.config.{env}.ts`),优先级 `FAAPI_ENV > NODE_ENV > 'development'`。启动时按 mode 兜底设置 `NODE_ENV` 是为了同步给生态下游(如 Next.js 运行时多处直接读 `process.env.NODE_ENV`),不覆盖用户显式设置。

## 设计原则

faapi 遵循"**只转译不检查类型**"的框架职责边界。框架用 esbuild 编译路由与配置文件,**不主动做 tsc 检查**。类型安全由三层保证:
