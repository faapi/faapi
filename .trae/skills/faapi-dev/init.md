...
```json
{
  "scripts": {
    "dev": "faapi",
    "build": "faapi build",
    "start": "NODE_ENV=production faapi",
    "typecheck": "tsc --noEmit"
  }
}
```

> 注:`start` 通过 `faapi` 命令启动(从 `node_modules/.bin/faapi` 软链)。如果全局未安装,用 `pnpm exec faapi`。
...
```bash
# 构建(编译 .ts → dist/.js + 生成 faapi-schema.js)
pnpm build
...
```bash
faapi build           # 构建 faapi
next build            # 构建 Next.js
NODE_ENV=production faapi   # 启动(自动用 prod 模式)
```
...
**解决**:先 `faapi build`,再 `NODE_ENV=production faapi`。
