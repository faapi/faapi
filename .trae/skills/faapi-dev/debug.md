...
遇到问题
  ↓
1. 先查源码确认(faapi TS 源码 + 业务项目自身源码两侧取证,定位文件+行号,不靠猜测)
  ↓
2. 有变通方案?
...
**取证口径**(2026-09 真实误判的教训):"源码依据"需覆盖两层——**业务项目自身源码**(先排除项目侧覆盖/配置/变通在起作用,如 JSDoc `@tool` / `@agent` 覆盖名、自定义中间件)和 **faapi TS 源码**(`packages/faapi/src/**`,而非 node_modules 里的 dist 产物——dist 只证明"发生了什么",证明不了"为什么/默认规则是什么")。"文档描述"与"构建产物"对不上时,先假设存在项目侧覆盖,查完输入侧再下"规则已变"的结论。典型反例见下方实战示例的「文档错误误判」。
...
#### 反例:文档错误误判(2026-09 真实案例,勿重蹈)

技能文档说 tool 名是 `<子目录>.<函数名>`,业务项目构建产物 `faapi-tools.js` 里却是裸目录名(`"name": "chapter", "functionName": "invoke"`)。取证查了框架 dist 的 executeTool(按 rawName 查清单)与构建产物,唯独没查项目自身源码,于是误判"框架命名规则已变",写下文档反馈 TODO。实际上项目 13 个 tool 全部在 handler JSDoc 里写了 `@tool <目录名>` 覆盖默认名(默认名 `chapter.invoke`),默认规则从未变。

**教训**:

- 产物反映的是覆盖后的**最终名**;「文档说 A + 产物是 B」与「规则变更」「项目侧覆盖」两个假设都相容,必须看到输入侧(项目源码)才能区分
- 同一机制只在框架内部 DDD 文档记载、技能文档缺位时尤其危险——覆盖的会话不留痕,核对的会话不知道。修复优先补全技能文档(见 [agent.md](./agent.md) 的「`@tool` 覆盖名」),而非只改一处
- 命名/行为类覆盖手段(`@tool` / `@agent` / 自定义中间件)应写入项目提交说明或注释,避免后续会话把它们当成框架默认行为反推规则
