...
/** coerceMap 公用函数源码（ESM export 格式，写入 faapi-helpers.js） */
const COERCE_MAP_HELPER: string
...
/** 检测代码是否引用了 coerceNumber / coerceBoolean / coerceMap / coerceSet 变量（决定是否生成 faapi-helpers.js） */
...
| `Map<K,V>` | `coerceMap` | `Array.isArray(v) ? new Map(v) : v instanceof Map ? v : (v && typeof v === "object" ? new Map(Object.entries(v)) : v)`（接受 entries 数组、Map 实例、普通对象；其他原值保留让 zod 报错） |
| `Set<T>` | `coerceSet` | `v instanceof Set ? v : (Array.isArray(v) ? new Set(v) : v)`（接受 Set 实例或数组；其他原值保留让 zod 报错） |

`Map` / `Set` 与 `number` / `boolean` 不同：在 `coerce=false`（body 场景）下也会生成 `z.preprocess` 包裹，因为 `JSON.parse` 出来的是数组/对象，必须还原为 Map/Set 实例才能交给 `z.map()` / `z.set()` 校验。`coerce=true` 仅额外对内部 number/boolean 字段加一层 `coerceNumber`/`coerceBoolean`。
