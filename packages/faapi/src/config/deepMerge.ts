 ...
 * - `compileConfig` build 时合并并生成 `.faapi/build/faapi-config.js`（通过 `DEEP_MERGE_SOURCE` 字符串内联）
 *
 ...
 *
 * 供 `compileConfig` 生成 `.faapi/build/faapi-config.js` 时内联：esbuild bundle 入口源码引用此字符串，
