...
这种「产物驱动」设计让 dev 和 prod 走完全一致的代码路径：差异仅是 `dist` 参数（dev = `.faapi/dev`，prod = `.faapi/build`），不存在 `if (isDev)` 分支。

## 使用场景

- `faapi` / `faapi dev` 启动时：`devCommand` 先调 `compileConfig` 生成 `.faapi/dev/faapi-config.js`，再调 `loadConfig(rootDir, '.faapi/dev')`
- `faapi build` 时：`buildCommand` 调 `compileConfig` 生成 `.faapi/build/faapi-config.js`
- `node .faapi/build/main` 生产启动时：`createProdApp` 调 `loadConfig(rootDir, '.faapi/build')` 读 build 产物
...
| `dist` | `string` | 产物目录（dev 为 `.faapi/dev`，prod 为 `.faapi/build`） |
