...
一句话概括：加载 faapi 配置产物，统一读 `<outDir>/faapi-config.js`，dev/prod 同路径无分支。
...
这种「产物驱动」设计让 dev 和 prod 走完全一致的代码路径：差异仅是 `outDir` 参数（dev = `.faapi/dev`，prod = `dist`），不存在 `if (isDev)` 分支。

## 使用场景

- `faapi` / `faapi dev` 启动时：`devCommand` 先调 `compileConfig` 生成 `.faapi/dev/faapi-config.js`，再调 `loadConfig(rootDir, '.faapi/dev')`
...
- 编程式调用：传 `outDir` 指定产物所在目录

## API

| 方法 | 说明 |
|------|------|
| `loadConfig(rootDir, outDir)` | 从 `<rootDir>/<outDir>/faapi-config.js` 加载配置产物，返回 `Partial<FaapiConfig> \| null` |
...
| `outDir` | `string` | 产物目录（dev 为 `.faapi/dev`，prod 为 `dist`） |

## 行为

1. 检查 `<rootDir>/<outDir>/faapi-config.js` 是否存在
