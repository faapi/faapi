...
构建前先 `compileConfig` + `loadConfig` 读应用行为配置，CLI 选项或环境变量读 `appDir`/`patterns`（build 时无 `dist/` 产物，先生成临时配置产物再读），随后执行 7 步：

1. **扫描源文件**（全量扫描 `appDir/**/*.ts`，排除测试文件和声明文件，作为编译 entryPoints）
2. **编译 TypeScript**（`compileBuildRoutes`：`bundle: false` 逐文件编译，与 dev 一致，打平 appDir 前缀）
...
7. **生成启动入口**（写入 `dist/main.js`：`import { createProdApp } from '@faapi/faapi'` + `createProdApp()` + `listen()`；`--port` / `--outDir` 选项写入 `main.js`，prod 启动时无需再设环境变量）

## 编译模式

`faapi build` 采用 `bundle: false` 逐文件编译，与 `faapi dev` 完全一致，差异仅在 `outDir`（build → `dist/`，dev → `.faapi/dev/`）。
...
`--port` 和 `--outDir` 选项会写入 `main.js`，prod 启动时无需再设环境变量：

```bash
faapi build --port 8080 --outDir build
# 生成 build/main.js:
# const app = await createProdApp({ outDir: 'build' });
...
- **编译范围**：全量扫描 `appDir/**/*.ts` 作为 entryPoints。
- **别名重写**：esbuild `onLoad` 读取源文件后，用正则把 `import/export ... from 'alias'` 和 `import('alias')` 中的别名 specifier 替换为产物相对路径（`.js` 后缀），再交给 esbuild 转译。
- **appDir 前缀剥离**（`compileConfig` 用）：config 文件位于 rootDir 时,引用 appDir 内模块的相对 import 被重写为剥离前缀的产物路径,使 config 与 routes 共享同一份模块产物。
- 无 tsconfig / paths 时插件仍生效（相对路径加 `.js` 后缀是 Node ESM 必需）。

## 相关模块

- `compileBuildRoutes.ts` - TypeScript 逐文件编译（与 dev 一致）
- `compileConfig.ts` - 配置文件编译合并（build 时输出 `dist/faapi-config.js`，两步编译保证 instanceof 跨边界生效）
