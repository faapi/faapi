...
dev 和 prod 走完全一致的读产物代码路径（`createAppBase`），差异仅由 `FAAPI_DIST` 环境变量（路径参数）驱动，无 `if (isDev)` 控制流分支。`NODE_ENV` 用于 `loadEnv` 选择 `.env.{env}` 文件。
...
| [loadEnv.ts](./loadEnv.ts) | `.env` 系列文件加载：按 Next.js 约定加载到 `process.env`（dev/prod 启动时调用） |
...
node dist/main.js（运行 build 阶段生成的启动入口）
  → 兜底 NODE_ENV=production（未显式设置时）+ loadEnv(cwd)（加载 .env 系列文件到 process.env）
...
`dist/main.js` 是 `faapi build` 自动生成的启动入口（零入口设计：用户无需编写 main.ts），内部 `import { createProdApp, loadEnv } from '@faapi/faapi'` + 兜底 `NODE_ENV=production` + `loadEnv(cwd)` + `createProdApp()` + `listen()`。`createProdApp()` 统一读 `dist/` 产物三元组，与 dev 模式的 `createDevApp()` 走完全相同的 `createAppBase` 代码路径，差异仅在 `FAAPI_DIST` 值不同。
