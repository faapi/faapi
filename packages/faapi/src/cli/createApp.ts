 ...
 * - `createProdApp`：prod 入口（精简，无 reloadRoutes），由 `.faapi/build/main.js` 内部调用
 ...
 * prod 由 `faapi build` 自动生成 `.faapi/build/main.js` 启动入口。`createApp`/`createProdApp` 主要供自定义编程式启动场景使用。
 *
