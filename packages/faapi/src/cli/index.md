...
| 产物 | dev | prod |
|------|-----|------|
| `*.js`（路由/middleware 编译） | `.faapi/**/*.js` | `dist/**/*.js` |
| `faapi-config.js`（配置合并产物） | `.faapi/faapi-config.js` | `dist/faapi-config.js` |
...
- `buildCommand.ts` - 构建逻辑（逐文件编译（bundle: false） + 生成产物三元组到 `dist/` + 生成 `dist/main.js` 启动入口）
