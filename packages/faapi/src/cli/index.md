...
**命令分发**：
- `faapi build` → 调用 `buildCommand`（bundle 编译到 `dist/` + 生成产物三元组 + 生成 `dist/main.js` 启动入口）
...
`createApp` / `createProdApp` / `createDevApp` 主要供编程式调用场景使用（如自定义启动器），`dist/main.js` 内部也调用它们完成启动。

## 产物三元组

dev 和 prod 生成完全一致的产物三元组，差异仅由 `FAAPI_OUT_DIR` 环境变量驱动：
...
`faapi.config.ts` 由 `compileConfig` 用 esbuild 编译合并为单个 `faapi-config.js`（写入 outDir），运行时 `loadConfig` 直接 import 产物，零编译、零合并：
...
- 用户执行 `faapi build`（构建到 `dist/` + 生成 `dist/main.js` 启动入口，不启动服务器）
- 用户执行 `node dist/main`（启动生产服务器，读 `dist/` 产物三元组）
- Node.js 解析 shebang 执行

## 相关模块

- `devCommand.ts` - dev 模式编排（设 `FAAPI_OUT_DIR` + 编译 + 生成产物 + 调 `createDevApp` + watcher）
- `buildCommand.ts` - 构建逻辑（bundle 编译 + 生成产物三元组到 `dist/` + 生成 `dist/main.js` 启动入口）
- `createAppCore.ts` - dev/prod 共享的 `createAppBase` 编排核心
- `createDevApp.ts` - dev 入口（含 reloadRoutes 热替换）
- `createProdApp.ts` - prod 入口（精简，由 `dist/main.js` 内部调用）
...
- `loadConfig.ts` - 加载 `faapi-config.js` 产物（统一读 outDir）
