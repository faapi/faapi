...
describe('createAppBase', () => {
  ...
  describe('inject raw 类型保真（handler 原始返回值）', () => {
    let tempDir: string;

    beforeEach(() => {
      tempDir = join(tmpdir(), `faapi-raw-${Date.now()}-${Math.random().toString(36).slice(2)}`);
      mkdirSync(tempDir, { recursive: true });
      invalidateMiddlewareCache();
      invalidateProgramCache();
    });
    ...
    /** 编译产物 + 创建 app（configContent 可选，中间件拦截用例需要） */
    async function setupApp(configContent: string, handlerContent: string) {
      if (configContent) {
    ...
    it('handler 返回 Date 富类型：raw 保真为 Date 实例，body 仍为 ISO 字符串（HTTP 语义不变）', async () => {
      const app = await setupApp(
        '',
        `export function GET() {
  return { id: 1, updatedAt: new Date('2026-01-01T00:00:00.000Z') };
}\n`,
      );

      const res = await app.inject({ method: 'GET', path: '/api/hello' });
      expect(res.status).toBe(200);
      // body：JSON 反序列化结果（Date → ISO 字符串）——与真实 HTTP 客户端一致，行为不变
      expect(res.body).toEqual({ data: { id: 1, updatedAt: '2026-01-01T00:00:00.000Z' } });
      // raw：handler 原始返回值——Date 富类型不丢（RSC 同进程取数类型保真）
      const raw = res.raw as { id: number; updatedAt: Date };
      expect(raw).toBeDefined();
      expect(raw.updatedAt).toBeInstanceOf(Date);
      expect(raw.updatedAt.getTime()).toBe(new Date('2026-01-01T00:00:00.000Z').getTime());
      expect(raw.id).toBe(1);

      await app.close();
    });

    it('handler 返回 null：raw 为 null（区别于 undefined 的未捕获）', async () => {
      const app = await setupApp('', `export function GET() { return null; }\n`);
    ...
    it('handler return ctx.ok(data)：Response 形态不捕获，raw 为 undefined', async () => {
      const app = await setupApp('', `export function GET(ctx) { return ctx.ok({ id: 1 }); }\n`);
    ...
    it('handler return ctx.fail(...)：主动错误不捕获，raw 为 undefined', async () => {
      const app = await setupApp(
        '',
        `export function GET(ctx) {
  return ctx.fail({ status: 404, code: 'USER_NOT_FOUND', message: 'nope' });
}\n`,
      );

      const res = await app.inject({ method: 'GET', path: '/api/hello' });
      expect(res.status).toBe(404);
      expect(res.body).toEqual({ error: { code: 'USER_NOT_FOUND', message: 'nope' } });
      expect(res.raw).toBeUndefined();

      await app.close();
    });

    it('handler 抛错：错误响应不捕获，raw 为 undefined', async () => {
      const app = await setupApp('', `export function GET() { throw new Error('boom'); }\n`);
    ...
    it('中间件拦截（未调 next）：handler 未执行，raw 为 undefined；放行请求 raw 正常捕获', async () => {
      const app = await setupApp(
  ...
  middlewares: [
    async (ctx, next) => {
      if (!ctx.headers.get('authorization')) {
        return ctx.fail({ status: 401, message: 'token required' });
      }
      await next();
    },
  ],
...
`,
        `export function GET() { return { secret: 'data' }; }\n`,
      );

      // 拦截：handler 未执行 → raw undefined
      const denied = await app.inject({ method: 'GET', path: '/api/hello' });
      expect(denied.status).toBe(401);
      expect(denied.raw).toBeUndefined();
      ...
      expect(allowed.raw).toEqual({ secret: 'data' });

      await app.close();
    });

    it('404 无匹配路由：handler 未执行，raw 为 undefined', async () => {
      const app = await setupApp('', `export function GET() { return { ok: true }; }\n`);

      const res = await app.inject({ method: 'GET', path: '/api/missing' });
      expect(res.status).toBe(404);
      expect(res.raw).toBeUndefined();
