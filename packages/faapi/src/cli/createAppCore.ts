...
export { loadAndHydrateTools, loadAndHydrateAgents, loadAndHydrateTasks } from './manifestLoader';
...
/**
 * listen 阶段打印路由/tool/agent/task 清单（启动日志）
 */
 ...
 *
 * 模块构成：编排主流程在本文件；清单装载见 [manifestLoader](./manifestLoader.md)、
 * 单例与停机信号见 [appSingleton](./appSingleton.md)、无服务器注入见
 * [injectMock](./injectMock.md)。
 */
export async function createAppBase(options?: CreateAppOptions): Promise<{
  ...
  // 水合 tool / agent / task 清单（可选产物——缺失时对应注册表保持空）
  ...
  const app: AppBase = {
    ...
    async listen(listenPort?: number): Promise<Server> {
      ...
      return new Promise<Server>((resolve, reject) => {
        ...
        server.listen(actualPort, async () => {
          ...
          logStartupSummary(sorted, wsRoutes, tools, agents, taskMetas, p);
    ...
    inject(injectOpts?: InjectOptions): Promise<InjectResponse> {
      return performInject(server, injectOpts);
    },
