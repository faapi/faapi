...
### inject 的 body 序列化契约（与真实 HTTP 完全一致）

`inject` 走完整请求链路，handler 返回值经统一序列化（`src/utils/stringifyJson.ts`）再由 `JSON.parse` 反序列化为 `body`——**与真实 HTTP 客户端拿到的完全相同**，这是有意设计：接口的线上格式即契约，同进程取数与跨进程取数看到的 body 没有第二种形态，业务方按转换后的类型消费或在边界自行还原。

序列化转换规则（完整表见 [../utils/stringifyJson.md](../utils/stringifyJson.md)）：
...
async function Page() {
  ...
  const data = res.body;  // 已解析，无需 await res.json()；Date 字段为毫秒时间戳等序列化契约见「inject 的 body 序列化契约」
...
- `inject` body 按类型区分语义（string/Buffer 原样透传，对象 JSON.stringify），调用方 content-type 优先——见「inject 的 body 语义」；body 序列化契约与真实 HTTP 一致——见「inject 的 body 序列化契约」
