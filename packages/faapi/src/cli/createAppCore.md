...
- 不负责编译 TypeScript——编译由 `faapi dev`（esbuild → `.faapi/dev/`）和 `faapi build`（→ `.faapi/build/`）负责
...
`dist` 由 `process.env.FAAPI_DIST` 决定：`faapi dev` 设为 `.faapi/dev`，`node .faapi/build/main` 不设（默认 `.faapi/build`）。
