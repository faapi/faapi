...
`createAppBase` 不负责编译 TypeScript——编译由 `faapi dev`（`compileDevRoutes` 增量编译到 `.faapi/dev/`）和 `faapi build`（`compileBuildRoutes` bundle 编译到 `dist/`）负责。`createAppBase` 只消费已编译的产物三元组：

| 产物 | dev 模式路径 | prod 模式路径 | 由谁生成 |
|------|------------|-------------|---------|
| 路由/middleware 编译 `.js` | `.faapi/dev/**/*.js` | `dist/**/*.js` | `compileDevRoutes` / `compileBuildRoutes` |
| 配置产物 | `.faapi/dev/faapi-config.js` | `dist/faapi-config.js` | `compileConfig` |
| 路由清单 | `.faapi/dev/faapi-routes.js` | `dist/faapi-routes.js` | `generateRouteArtifacts` / `buildCommand` |
| schema 模块 | `.faapi/dev/**/zod.js` | `dist/**/zod.js` | `generateSchemaFiles` |
...
`createAppBase` 内部无 `if (isDev)` 分支：统一水合 `faapi-routes.js` 路由清单，统一 `loadConfig(outDir)` 读配置，统一按需 import `zod.js` 做 zod safeParse。
...
3. `scanRoutes(rootDir, patterns, appDir, outDir)` 重新扫描
...
- `../config/loadConfig.ts` - `createAppBase` 统一读 `<outDir>/faapi-config.js`
