...
- **`createProdApp`**：prod 专用，精简（无 reloadRoutes），由 `.faapi/build/main.js` 内部调用
...
`createAppBase` 不负责编译 TypeScript——编译由 `faapi dev`（`compileDevRoutes` 增量编译到 `.faapi/dev/`）和 `faapi build`（`compileBuildRoutes` bundle 编译到 `.faapi/build/`）负责。`createAppBase` 只消费已编译的产物三元组：

| 产物 | dev 模式路径 | prod 模式路径 | 由谁生成 |
|------|------------|-------------|---------|
| 路由/middleware 编译 `.js` | `.faapi/dev/**/*.js` | `.faapi/build/**/*.js` | `compileDevRoutes` / `compileBuildRoutes` |
| 配置产物 | `.faapi/dev/faapi-config.js` | `.faapi/build/faapi-config.js` | `compileConfig` |
| 路由清单 | `.faapi/dev/faapi-routes.js` | `.faapi/build/faapi-routes.js` | `generateRouteArtifacts` / `buildCommand` |
| schema 模块 | `.faapi/dev/**/zod.js` | `.faapi/build/**/zod.js` | `generateSchemaFiles` |

`dist` 由 `process.env.FAAPI_DIST` 决定：

- `faapi dev` 启动时设为 `.faapi/dev` → 读 dev 产物
- `node .faapi/build/main` 不设 → 默认 `.faapi/build`，读 prod 产物

`createAppBase` 内部无 `if (isDev)` 分支：统一水合 `faapi-routes.js` 路由清单，统一 `loadConfig(dist)` 读配置，统一按需 import `zod.js` 做 zod safeParse。

## 使用场景

### prod 模式（.faapi/build/main.js 内部）

`faapi build` 生成 `.faapi/build/main.js` 启动入口，内部调用 `createProdApp` + `listen`，读 `.faapi/build/` 产物：

```js
// .faapi/build/main.js（由 faapi build 自动生成）
...
`faapi dev` 直接调用 `createDevApp` + `listen`，读 `.faapi/dev/` 产物：
...
- `./buildCommand.ts` - `faapi build` 命令，生成 `.faapi/build/main.js` 启动入口（内部调用 `createProdApp` + `listen`）
