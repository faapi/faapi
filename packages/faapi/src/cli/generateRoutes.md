...
一句话概括：build 时序列化路由清单为 JS 模块，start 时水合还原（加载中间件）。
...
- `faapi start`：`import` 读取清单 + `hydrateRoutes` 加载中间件还原 `RouteManifest`

## 序列化策略

- `filePath`：dev 形式（`api/user/handler.ts`）→ prd 形式（`dist/api/user/handler.js`）
- `middlewares`/`injectors`：函数无法序列化，改为 `middlewarePaths`（中间件文件绝对路径列表，根在前）
- `middlewarePaths`：build 时向上查找 `middlewares.{ts,js}`（优先 .ts），转为 prd 形式绝对路径
...
- `startCommand.ts` - start 时调用读取与水合
