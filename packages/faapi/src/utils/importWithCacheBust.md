...
一句话概括：动态 import ESM 模块，watch 模式下通过 URL query 参数（`?t=<timestamp>`）绕过 ESM 缓存。非 watch 模式等价于普通 `import()`。
...
`filePath` 须为绝对路径，内部用 `pathToFileURL` 转为 `file://` URL。无显式 try/catch——`import()` 失败（模块不存在/语法错误）异常直接向上传播。
