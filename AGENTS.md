...
build: CLI → compileBuildRoutes → .faapi/build/ + .faapi/build/main.js │
prod:  node .faapi/build/main → createProdApp() → .faapi/build/        ┤→ Server → Router → Loader → Runtime → Response
                                                                       ↓                ↓
                                                                   Injection        Validator
                                                                       ↓                ↓
                                                                     AST            Middleware
...
生产模式：`faapi build` 生成产物 + `.faapi/build/main.js` 启动入口，`node .faapi/build/main` 调 `createProdApp()` 自动水合路由清单。

框架采用零入口设计——用户无需编写 `main.ts`：dev 由 CLI 内部编排，prod 由 build 阶段生成 `.faapi/build/main.js` 启动入口。
...
- **prod**：`faapi build` 生成 `.faapi/build/main.js` 启动入口（内部 import `createProdApp` + `listen`），`node .faapi/build/main` 直接启动

框架采用零入口设计——用户无需编写 `main.ts`：dev 由 CLI 内部编排，prod 由 build 阶段自动生成 `.faapi/build/main.js` 启动入口。用户自定义启动逻辑（初始化数据库、注册信号处理等）通过 `faapi.config.ts` 的 `lifecycle.onReady` / `onClose` 钩子实现，dev/prod 都执行。
...
# 生产模式
faapi build                    # 构建（bundle 编译 .ts → .faapi/build/*.js + 编译合并配置 → .faapi/build/faapi-config.js + 生成 .faapi/build/faapi-routes.js + 每个 handler 的 zod.js + 生成 .faapi/build/main.js 启动入口）
node .faapi/build/main         # 启动生产服务器（main.js 内部调 createProdApp 读 .faapi/build/ 产物三元组）
```

`createApp` / `createProdApp` / `createDevApp` 主要供编程式调用场景使用（如自定义启动器、测试场景），`.faapi/build/main.js` 内部也调用它们完成启动。
...
- **框架元信息**（port、dist）通过环境变量传入，不放在 config 内：
  - `PORT`：服务端口，默认 3000
  - `FAAPI_DIST`：产物输出目录（实际目录），dev 为 `<dist>/dev`（默认 `.faapi/dev`），prod 为 `<dist>/build`（默认 `.faapi/build`）。CLI `--dist` 选项是产物根目录（默认 `.faapi`），dev/prod 实际产物输出到 `<dist>/dev`、`<dist>/build`。
...
# prod 模式
PORT=8080 node .faapi/build/main
```
...
| 产物 | dev 模式 | prod 模式 |
|------|---------|----------|
| `*.js`（路由/middleware/项目模块编译） | `.faapi/dev/**/*.js` | `.faapi/build/**/*.js` |
| `faapi-config.js`（配置入口产物，import config 源产物 + 内联 deepMerge） | `.faapi/dev/faapi-config.js` | `.faapi/build/faapi-config.js` |
| `faapi.config.js`（config 源编译产物，保留相对 import 指向项目模块） | `.faapi/dev/faapi.config.js` | `.faapi/build/faapi.config.js` |
...
| `faapi-helpers.js`（coerce 公用函数，仅有 number/boolean 字段时生成） | `.faapi/dev/faapi-helpers.js` | `.faapi/build/faapi-helpers.js` |
| `main.js`（启动入口，仅 prod） | — | `.faapi/build/main.js` |

- `faapi` / `faapi dev`：dev 模式，`devCommand` 设 `FAAPI_DIST=.faapi/dev`（CLI `--dist` 默认 `.faapi`，实际产物目录为 `<dist>/dev`），`compileDevRoutes` 逐文件编译 `.ts` → `.faapi/dev/*.js`（`bundle: false`，启动快、增量编译），调 `compileConfig` 两步编译生成 `.faapi/dev/faapi-config.js`（config 源 + 项目模块逐文件编译 + 入口 bundle external），调 `generateRouteArtifacts` 生成 `faapi-routes.js` + `zod.js`，调 `createDevApp()` + `listen()`（含 `reloadRoutes` 热替换能力），watch 文件变化（增量编译 + 重生成 `faapi-config.js` + 调 `app.reloadRoutes()` 热替换路由）
- `faapi build`：构建，`compileBuildRoutes` 逐文件编译（`bundle: false`，与 dev 一致，打平 src 前缀）→ `.faapi/build/*.js` + `compileConfig` 两步编译合并配置 → `.faapi/build/faapi-config.js` + 生成 `.faapi/build/faapi-routes.js` + 每个 handler 的 `zod.js` + 生成 `.faapi/build/main.js` 启动入口（零入口设计：内部 import `createProdApp` + `listen`），不启动服务器
- `node .faapi/build/main`：生产模式，直接运行 `.faapi/build/main.js`，`createProdApp()` 读 `FAAPI_DIST`（未设置时默认 `.faapi/build`），水合 `.faapi/build/faapi-routes.js` 路由清单，`loadConfig` 读 `.faapi/build/faapi-config.js`，运行时按需 import `zod.js` 做 zod safeParse
...
- **`process.env.NODE_ENV` 处理**：不再用 `define` 在编译时替换。业务代码中的 `process.env.NODE_ENV` 表达式保留，运行时读取环境变量。dev 时 `devCommand` 兜底设为 `'development'`，build 产物由部署环境注入（或启动 `NODE_ENV=production node .faapi/build/main` 显式设置）。
...
启动时按 mode 兜底设置 `NODE_ENV`（仅在未显式设置时，不覆盖用户意图）：`faapi`/`faapi dev` → `development`，`node .faapi/build/main` → 由用户自行设置。业务代码中的 `process.env.NODE_ENV` 表达式在编译阶段保留（不传 `define`），运行时读取环境变量。如果业务配置中有运行时读取 `process.env.NODE_ENV` 的逻辑（如 `onReady` 钩子），需启动时显式设置或由部署环境注入。
...
- 框架采用零入口设计——用户无需编写 `main.ts`：dev 由 `faapi dev` 内部编排（调 `createDevApp` + `listen`），prod 由 `faapi build` 自动生成 `.faapi/build/main.js` 启动入口（内部 import `createProdApp` + `listen`），`node .faapi/build/main` 直接启动。用户自定义启动逻辑通过 `faapi.config.ts` 的 `lifecycle.onReady` / `onClose` 钩子实现。`createApp` / `createProdApp` / `createDevApp` 主要供编程式调用场景使用（如自定义启动器、测试场景），`createApp` 为 `createProdApp` 的向后兼容别名。
...
- AST 提取 `RuntimeType` 结构化类型描述，再生成 zod schema JS 代码（`zod.js` 文件）：
  - 每个 handler 生成一个 `zod.js`，与 `handler.js` 同级（如 `.faapi/build/api/hello/zod.js`）。
  - dev：`devCommand` 启动时 + watch 时调 `generateSchemaFiles` 生成 `zod.js` 到 `.faapi/dev/`。
  - prd：`faapi build` 调 `generateSchemaFiles` 生成 `zod.js` 到 `.faapi/build/`。
