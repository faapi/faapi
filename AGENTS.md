...
- **任务定义（文件约定）**：`src/tasks/<name>/task.ts`，导出 `task` 元信息对象（`concurrency` / `retries` / `timeoutMs` / `graceMs` / `cron`，均可选；`graceMs` 为取消宽限期毫秒数，仅声明 `timeoutMs` 的隔离任务生效，默认 5s）+ `run(payload, taskCtx)`；`run` 首参类型（如 `Payload` interface）走 AST → zod 代码生成，入队时校验（不合法抛 `ValidationError`）
...
- **任务侧注册表只读视图（TaskContext.registries）**：两条执行路径均注入 `taskCtx.registries`（agent/tool/skill 元数据的只读查询视图，不含 hydrate/clear 写接口），任务内组装/调用 agent 不再依赖 `getApp()`——注册表为 app 实例级，全局访问器读的是 app 启动从不水合的默认实例，`getApp()` 在隔离 worker 内也不可用（globalThis 独立），任务侧曾没有任何注册表访问路径。进程内传活引用（`createTaskRegistriesView`）；隔离路径注册表对象含函数闭包不可跨线程，由语义层生成纯数据快照（agents 含 filePath/hasRun 完整元数据 + tools + skills）postMessage 传入、worker 内重建视图——**快照语义**：视图反映派发时刻的注册表，执行中途 reload/DB skill 变更不影响当次执行。详见 `packages/faapi/src/task/taskTypes.md`
...
- **管理能力（TaskClient 扩展 + TaskDriver 可选方法）**：`list(name?)`（同步，本进程快照）、`listQueued(name?)`（持久化队列视图，驱动记录与本进程记录按 id 合并、本进程优先）、`cancel(name, id)`、`retry(name, id)`。驱动方法全部可选、按队列系统能力实现——BullMQ 全支持（list 状态映射 / cancel=job.remove / retry=job.retry）；pg-boss v10 有 `cancel`/`resume` 无批量列出 API（list 未实现，listQueued 显式抛错），驱动未实现时显式抛错不静默降级。能力边界见 `packages/faapi/src/task/driverTypes.md`
...
- **驱动必填（memory 内置驱动已移除）**：存储/消费/重试/停机抽象为 `TaskDriver` 接口，由独立子包提供——`@faapi/task-pgboss`（Postgres）/ `@faapi/task-bullmq`（Redis），主包按 `config.task.driver` 动态加载（选项经 `task.pgboss` / `task.bullmq` 透传），未安装显式报错。**存在任务清单（`src/tasks/**/task.ts`）时必须显式配置 `config.task.driver`，否则启动报错**；无任务清单的项目不加载驱动（`idleTaskDriver` 占位，零任务项目无需安装驱动子包）。业务方写法（文件约定任务 + `tasks.enqueue`）跨驱动完全一致
- **明确取舍**：框架不提供"可丢任务"的进程内默认实现——持久化、多实例防重跑由所选驱动承载；HTTP 手动触发端点留给后续
...
个人技能库 `faapi-dev` 技能文档的更新**不由框架侧自动同步**：由维护者在正式发版后手动同步、自行触发。框架开发时不在 faapi-dev 文档中代写内容或预测发版版本。
